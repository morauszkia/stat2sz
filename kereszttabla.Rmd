...
Az első nagyobb téma ebben a félévben a &chi;&sup2; (Khí-négyzet) tesztek lesznek. Ezek közül az első a kereszttáblaelemzés.

Először elhelyezzük a módszert a többi között. Ugyan erre a kereszttábla esetében nem lesz szükség, de a két változó közül, amelyek között vizsgáljuk az összefüggést, tegyünk különbséget: az egyik a **független**, a másik pedig a **függő** változó. Arra számítunk, hogy a független változó értéke hatással van a függő változó értékére.
Pl. az, hogy valaki milyen nemű (független változó) hatással van arra, tetszik-e neki egy film (függő változó)

Attól függően, milyen mérési szintűek a változóink, más-más próbával vizsgálható az összefüggésük.
...
A kereszttábla esetében olyan változópárok közötti összefüggést vizsgálunk, amelyek **mindkét tagja nominális vagy ordinális szintű**. Ilyen például a fenti példa a nemmel és azzal, tetszett-e nekik egy film.
...
- Ha az egyik változó sok kategóriából áll, míg a másik kevesebből, érdemes előbbit tenni a sorokba, mert úgy kevésbé széles, de hosszú táblázatot kapunk.
- Egyesek javasolják, hogy a független változó kerüljön az oszlopokba. Ha ehhez tartják magukat, akkor lehet, hogy megkönnyítik, de lehet, hogy megnehezítik a saját életüket. A fenti példában, az első változatban épp a sorok mutatják a független változó különböző értékeit (férfi vagy nő) és az oszlopok a függő változóét (tetszett-e a film).
...
Ebből a táblázatból közvetlenül kiolvasható és összevethető, hogy a férfiak és a nők körében külön-külön milyen arányban voltak, akiknek tetszett/nem tetszett az adott film. Azt eltérés nagysága persze továbbra is csak a mintára vonatkozik, és ebből még nem egyértelmű, hogy a populációban is van különbség.
...
2. **Oszlopszázalék**
Lehetőség van oszloponként is százalékolni. Ebben az esetben a cellaelemszámokat az oszlopösszegekkel osztjuk. Ebben az esetben ez talán kevésbé hat természetesen, de olyan kérdésekre kapunk választ, hogy a filmet szeretőknek hány százaléka volt nő a mintában? (Válasz: 45%) Azoknak, akik nem szerették a filmet, hány százalékuk volt férfi? (30%)
...
3. **Cellaszázalék**:
A cellaelemszámokat a mintaelemszámmal elosztva megkapjuk, hogy az egyes cellákban a minta hány százaléka van. Ezt a kapcsolat elemzésében kevésbé használjuk.
...
Tudjuk, hogy független események esetében az együttes bekövetkezés valószínűsége egyenlő a két esemény valószínűségének a szorzatával ($P(A*B)=P(A)\times P(B)$). Az elméleti kereszttáblát úgy számítjuk ki, hogy a mintanagyságot, illetve a marginálisokat (a két változó eloszlását külön-külön) meghagyjuk, csak a belső cellákat számoljuk újra, feltételezve a függetlenséget.
...
Tudjuk, hogy függetlenség esetén $P(F*T) = P(F) \times P(T)=0,45 \times 0,6 = 0,27 = 27\%$
...
$P(F*T)\times 100 = P(F)\times P(T) \times 100 =\frac{45}{100} \times \frac{60}{100} \times 100 = \frac{45\times60}{100}$
...
$$e_{i,j}=\frac{m_{i*}\times m_{*j}}{N}$$

A vizsgált kereszttábla négy cellájára ez:  
$e_{1,1}=\frac{45\times 40}{100}=18$  
$e_{1,2}=\frac{45\times 60}{100}=27$  
$e_{2,1}=\frac{55\times 40}{100}=22$  
$e_{2,2}=\frac{55\times 60}{100}=33$  
...
Képlete:
$$\chi^{2}=\sum_{i = 1}^{s}\sum_{j = 1}^{o}{\frac{(m_{i,j}-e_{i,j})^2}{e_{i,j}}}$$
...
Ha a nullhipotézis igaz, a fenti próba &chi;&sup2; eloszlást követ, amelynek szabadságfoka $df = (s-1)\times(o-1) = (2-1)\times(2-1)= 1$

<!-- IDE CHI2 ELOSZLÁS ÁBRA -->

A &chi;&sup2;-eloszlás táblázatában az 1-es szabadságfoknak megfelelő 1. sort nézzük. Ez a próba **mindig egyoldali**, tehág csak egy felső határértéket kell kikeresni. Mivel egyoldali, mindig az 1 - &alpha; oszlopot nézzük (&alpha; a szignifikanciaszint), vagyis:  
...
A 6,06-os tesztérték látható, hogy még 2,5%-os szint mellett is szignifikáns lenne, 1%-os szint mellett viszont már nem. Az SPSS ehhez a próbához is kiszámítaná a pontos szignifikanciaszintet, amely ebben az esetben `r test$pvalue`.  

Ezt úgy értelmezhetnénk, hogy igaz nullhipotézis (teljes függetlenség) esetén annak a valószínűsége, hogy 6,06-os vagy annál nagyobb tesztértéket kapjunk `r test$pvalue*100`%.
...
A &chi;&sup2; próba csak arra vonatkozik, hogy állítható-e a minta alapján, hogy a populációban is van bármilyen összefüggés a két változ2ó között, vagy elképzelhető, hogy a populációban a két változó egyáltalán nem függ össze.  
...
Ezek közül a legfontosabbak, amelyek kiszámítását is megnézzük, a &phi;, a Cramer-féle V, a PRE (másképpen Lambda) és a Gamma mutató.

### Phi (&phi;)
Olyan kereszttáblák esetén használható, amelyek egyik dimenziója (a sorok vagy oszlopok száma) 2. (A másik dimenzió akármennyi lehet, tehát pl. 2x10-es kereszttáblánál is jó a &phi; és 7x2-esnél is, mivel egyik kiterjedésük 2)  
...
Képlete:
$$Cramer V = \sqrt{\frac{\chi^{2}}{N*(min(s;o)-1)}}$$
...
A neve a Proportional Reduction of Error-ból ered, ami kifejezi a mutató logikáját is.
...
Hogy ne kelljen annyit visszalapozni, álljon itt ismét a megfigyelt kereszttábla:
```{r observed} 
data %>%
  tabyl(nem, tetszett) %>%
  adorn_totals(where = c("row", "col"), name = "összesen") %>%
...
Az a célunk, hogy minél többen jó helyre kerüljenek. 
...
2. lépés: Ezután megnézzük, mennyit segít a nem ismerete, hogy kevesebb embert tegyünk rossz kategóriába. Nézzük soronként:  
...
Ezek a mutatók is 0 és 1 (=100%) közötti értékeket vehetnek fel. 0 a leggyengébb, 1 pedig a legerősebb összefüggés. Előnye, hogy olyankor is használható, ha nem teljesülnek a &chi;&sup2; próba feltételei, és ezért a &phi; és a Cramer-féle V értékek nem megbízhatók.
...
### Gamma
Ordinális változók közötti kapcsolat számszerűsítésére szolgál, ezért a fenti példában nem használható. A későbbiekben megnézzük, hogyan működik. Ehhez nyissák meg a következő leckét!
...
Ezek önmagában nem mondják meg, hogy az eltérés jelentős-e. Pl. ha az eltérés a várhatótól 20, akkor ez jelentős lehet egy 100 fős minta esetében, de jelentéktelen egy 1000 fős mintánál. A korrigált standardizált reziduálisok (*adjusted standardized residual*) alkalmasak arra, hogy megállapítsuk, az eltérés az adott cella esetében szignifikáns-e. Jelöljük $r_{ij}^{(a)}$-val ezt a reziduálist.  
$$r_{ij}^{(a)}=\frac{m_{ij}-e{ij}}{\sqrt{e_{ij}\times \left(1-\frac{m_{i*}}{N}\right)\times\left(1-\frac{m_{*j}}{N}\right)}}$$  
Pl.  
$r_{11}^{(a)}=\frac{12-18}{\sqrt{18\times \left(1-\frac{45}{100}\right)\times\left(1-\frac{40}{100}\right)}}=-2,46$  
$r_{12}^{(a)}=\frac{33-27}{\sqrt{27\times \left(1-\frac{45}{100}\right)\times\left(1-\frac{60}{100}\right)}}=2,46$  
stb.  
